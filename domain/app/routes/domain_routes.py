...
def create_domain():
    ...
    try:
        ...
        # 3. Salva PDFs na pasta local uploads
        for file in pdf_files:
            if file and file.filename.endswith('.pdf'):
                ...
                query_pdf = """
                    ...
                    VALUES (%s, %s, %s);
                """
                cursor.execute(query_pdf, (filename, save_path, new_domain_id))

        # 4. Salva vídeos enviados na pasta local uploads
        for video_file in video_files:
            if video_file and video_file.filename.endswith('.mp4'):
                ...
                query_video_upload = """
                    ...
                    VALUES (%s, %s, %s);
                """
                cursor.execute(query_video_upload, (filename, save_path, new_domain_id))
...
def delete_domain(domain_id):
    ...
    try:
        ...
        for row in cursor.fetchall():
            try:
                file_path = resolve_local_file_path(row.get('path'), row['filename'])
                if file_path and os.path.exists(file_path):
                    os.remove(file_path)
            except Exception as e:
                logging.warning(f"Failed to delete file {row.get('path')}: {e}")
...
def download_pdf(pdf_id):
    ...
    try:
        ...
        file_path = resolve_local_file_path(pdf.get('path'), pdf['filename'])
        if not file_path:
            return jsonify({'error': 'File not found on local uploads folder'}), 404
...
def get_uploaded_video(video_id):
    ...
    try:
        ...
        file_path = resolve_local_file_path(video.get('path'), video['filename'])
        if not file_path:
            return jsonify({'error': 'Video not found on local uploads folder'}), 404
